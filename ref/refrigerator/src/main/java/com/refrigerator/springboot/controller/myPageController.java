...
public class myPageController {
	...
	public String recipeBook(Model model, Optional<Integer>page, HttpServletRequest request) {
		try {
			Member member = Member.guestMem();
			MemberDto dto = (MemberDto) request.getSession().getAttribute("user");
			if(dto!=null) {
			...
			Pageable pageable = PageRequest.of(page.isPresent()? page.get() :0,5);
			Page<ShowBoardDTO> recipeBoards = recipeBoardRepository.myPageRecipeBook(pageable, member);
			if(recipeBoards.getNumberOfElements()==0) {
				model.addAttribute("nonRecipeWriting", "등록된 게시물이 없습니다");
			}
			MemberImg memberImg = memImgRepository.findByMember(member);
