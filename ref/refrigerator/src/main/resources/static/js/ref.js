        ...
        $.ajax({
            ...
            success  : function(data){
                ...
                if ($('div.my_ref_style')[0]) {
                    $('.container').html(createhtml23(data));
                    startClick();
                }
            }
            ,error : function(request, status, error) {
...
function selectda(thiss){
    ...
    if(selList.indexOf(thiss.value) != -1){
        ...
        thiss.style.border="2px solid red";
    }
    checkSelectALl();
    console.log(selList);
}
        ...
        datalist.push(`
           <div class = "ing_btnList">
            <button class = "ing_click-btn" value = ${fruit.id}>
                ...
                <div style="display: inline-block;">
                <span class = "ingSpan" >${fruit.smallTags}</span><br>
                <input type="textarea" value="${fruit.memo}" style="  border-radius: 4px;  border: solid 1px;">
                </div>
                <input type="hidden" class ="backgrounds" value = ${fruit.ing_deadline}>
                 </div>`);
...
function createhtml2(data,i){
    ...
    datalist.push(`<div style="  margin-top: 14px; margin-left: 23px;">`);

    datalist.push(`
                <span style="  width: 143px;  display: inline-block; ">${data.text} </span>
                <input type="date"value = ${dataes} style=" padding: 8px; border-radius: 5px; border: 1px solid #ccc; background-color: #f2f2f2; font-size: 16px; height: 15px; width: 116px;    margin-right: 13px;"></input>
                <input type="text" value = '${data.memo}' style="padding: 8px;border-radius: 5px;border: 1px solid #ccc;background-color: #f2f2f2;font-size: 16px;height: 14px;width: 190px;font-style: normal;"></input>
                <span class="list-del" id = ${i} style=" margin-left: 5px; font-size: 29px;  position: relative; top: 3px;">×</span>
                `);
