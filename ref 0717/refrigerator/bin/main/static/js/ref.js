...
function selectda(thiss){
    ...
    if(selList.indexOf(thiss.value) != -1){
        ...
        if(deadLineCheck==-1){
			...
			thiss.style.border="1px solid red";
...
function select1(){
    ...
    for(let i = 0;i < spans.length;i++){
        spans[i].addEventListener('click',function(){
            ...
            let url = "/main/ingList";
            if(value =="전체"){ value = 0;}
            else if(value =="채소"){ value = 1;}
            else if(value =="육류"){ value = 2;}
            else if(value =="해산물"){ value = 3;}
            else if(value =="유제품"){ value = 4;}
            else if(value =="과일"){ value = 5;}
            else{ value = 6;}
            var token = $("meta[name='_csrf']").attr("content");
            var header = $("meta[name='_csrf_header']").attr("content");
            $.ajax({
        ...
        datalist.push(`
           <div class = "ing_btnList">
            <button class = "ing_click-btn" value = ${fruit.id}>
                ...
                <div style="display: inline-block;">
                <span class = "ingSpan" >${(fruit.ing_deadline==null)? fruit.smallTags : (now>new Date(fruit.ing_deadline.split('T')[0]))? '<span style="color:red; text-decoration:line-through;">'+fruit.smallTags+'</span>':fruit.smallTags}</span><br>
                <input type="textarea" value="${fruit.memo}" style="  border-radius: 4px;  border: solid 1px; ${(fruit.ing_deadline==null)?'color:gray;':(now>new Date(fruit.ing_deadline.split('T')[0]))?'color:red; background:black;':'color:gray;'}" readonly><br>
                <span class = "ingDeadLine">${(fruit.ing_deadline==null)?'<span style="font-weight : bolder;">유통기한 : </span> ----':(now>new Date(fruit.ing_deadline.split('T')[0]))? '<span style="font-weight : bolder; color:red;">유통기한 : </span>'+'<span style="color:red;">'+fruit.ing_deadline.split('T')[0]+'(종료)</span>':'<span style="font-weight : bolder;">유통기한 : </span>' + fruit.ing_deadline.split('T')[0]}</span><br>
                </div>
                <input type="hidden" class ="backgrounds" value = ${fruit.ing_deadline}>
                 </div>`);
...
function createhtml2(data,i){
    ...
    var yyyy = dataes.getFullYear();
    var mm = dataes.getMonth()+1 > 9 ? dataes.getMonth()+1 : '0' + (dataes.getMonth()+1);
    var dd = dataes.getDate() > 9 ? dataes.getDate() : '0' + dataes.getDate();
    ...
    datalist.push(`
                ...
                <input type="date" value = '${dates}' class="ingDate">
